...
from regime_specialist import (
    RegimeSpecialistLSTM,
    route_to_specialist,
    SPECIALIST_CONFIGS,
)
from saml_meta_learner import SAMLMetaLearner
from quantile_barrier import QuantileBarrierLearner
import ensemble_model as _em  # EXP15: for val-backtest threshold alignment
...
MIN_TEST_HOURS = int(
    3 * 730 * 0.80
)  # 1752 hours -- minimum for default/bear/covid modes
...
REGIME_CONFIG: Dict[str, Dict[str, Any]] = {
    "BULL": {
        ...
        "long_safety_sl": 0.045,
        "short_safety_sl": 0.045,
...
def cluster_stocks(
    ...
    for symbol in pool:
        try:
            ...
            df = pd.read_csv(
                fp, header=None, usecols=[0, 4], names=["Datetime", "Close"]
            )
            ...
            if anchor_end_date is not None:
                mask = pd.to_datetime(df.index, errors="coerce") <= pd.Timestamp(
                    anchor_end_date
                )
                df = df.loc[mask]
            if len(df) > 500:
    ...
    if len(series_dict) < group_size:
        raise ValueError(
            f"Too few stocks loaded ({len(series_dict)}) to form groups of {group_size}."
        )
    ...
    labels = fcluster(
        linkage(condensed, method="ward"), t=n_clusters, criterion="maxclust"
    )
...
def run_tournament(
    ...
    if use_walk_forward:
        ...
        print(
            f"  Validation window    : {wf_windows[0] if wf_windows else split_months}"
        )
    ...
    all_primary_sharpes = [
        r["val_sharpe"] for r in group_results if r["val_sharpe"] > -900
    ]
    avg_trades = (
        np.mean([r["val_trades"] for r in group_results if r["val_trades"] > 0])
        if group_results
        else 100
    )
    ...
    print(
        f"  DSR threshold        : {dsr_threshold:.3f}  (N={len(all_primary_sharpes)}, N_eff={_n_eff:.1f})"
    )
    ...
    if use_walk_forward:
        print(
            f"  Walk-forward windows : {n_walk_forward} (require positive in >={n_walk_forward - 1})"
        )
    print("=" * 60)
    ...
    for i, result in enumerate(group_results):
        passes_sharpe = (
            result["val_sharpe"] > effective_threshold and result["val_sharpe"] > 0.0
        )
        ...
        if acc_55_f is not None and result["val_sharpe"] > 3.5 and acc_55_f < 0.50:
        ...
        selected = (
            ...
            and passes_ticker_quality
            and passes_high_conf_acc
            and passes_monotonic_conf
            and passes_sharpe_consistency
        )

        reason = []
        if not passes_sharpe:
            reason.append(
                f"Sharpe {result['val_sharpe']:.3f} < {effective_threshold:.3f}"
            )
        if not passes_trades:
            reason.append(f"Trades {trades} < {MIN_VAL_TRADES}")
        if not passes_stability:
            reason.append(
                f"Stable in {n_positive}/{n_walk_forward} windows (need {required_positive})"
            )
        if not passes_ticker_quality:
            reason.append(f"Ticker quality {ticker_ratio * 100:.0f}% < 60%")
        if not passes_high_conf_acc:
            reason.append(
                f"acc_65 {acc_65_f:.3f} <0.50 on {int(trades_65_v)} hi-conf trades (anti-signal)"
            )
        if not passes_monotonic_conf:
            reason.append(
                f"acc_65 {acc_65_f:.3f} < acc_55 {acc_55_f:.3f} (confidence not discriminating)"
            )
        if not passes_sharpe_consistency:
            reason.append(
                f"Sharpe {result['val_sharpe']:.2f} >3.5 but acc_55 {acc_55_f:.3f} <0.50 (overfit Sharpe)"
            )

        status = "Y" if selected else "N"
        print(
            f"  Group {result.get('group_id', i):2d}: "
    ...
    if max_survivors and len(survivors) > max_survivors:
        ...
        for d in dropped:
            print(
                f"  Dropped (cap): Group {d['group_id']} Sharpe={d['val_sharpe']:+.3f}"
            )
...
def _write_tournament_csv(
    ...
    print("=" * 60)
    print(
        f"{'Grp':>4}  {'ValAcc':>7}  {'ValShrp':>7}  {'OvrAcc':>7}  "
...
def _load_survivors_from_csv(
    csv_path: str, selection_threshold: float
) -> List[Dict[str, Any]]:
    """
...
def run_combined_backtest(
    ...
    # EXP90: cross-seed robustness sweep showed replace_low_nonbear is the most
    # robust policy across seeds.  Keep all selected groups for the explicit BEAR
    # window; for non-BEAR windows replace the lowest-val selected group with the
    # highest-val non-selected group (if available).
    if train_anchor_date != BEAR_TRAIN_END and len(survivors) > 1:
        # Find lowest-val selected group
        low_survivor = min(survivors, key=lambda r: r.get("val_sharpe", 0.0))
        # Find highest-val non-selected group from tournament CSV
        replacement = None
        try:
            import csv as _csv
            _gt_path = Path("results/group_tournament.csv")
            if _gt_path.exists():
                _non_sel = []
                with open(_gt_path, newline="", encoding="utf-8", errors="replace") as _f:
                    for _r in _csv.DictReader(_f):
                        if _r.get("selected") != "Y":
                            try:
                                _vs = float(_r.get("val_sharpe", 0) or 0)
                                _pr = float(_r.get("ticker_positive_ratio", 0) or 0)
                            except (ValueError, TypeError):
                                continue
                            if _vs > 0 and _pr >= 0.5:
                                _non_sel.append({"group_id": int(_r["group_id"]), "stocks": _r["stocks"].split(","), "val_sharpe": _vs})
                _non_sel.sort(key=lambda x: x["val_sharpe"], reverse=True)
                if _non_sel:
                    replacement = _non_sel[0]
        except Exception:
            pass
        survivors = [r for r in survivors if r is not low_survivor]
        if replacement:
            survivors.append(replacement)
            print(
                "  Non-BEAR survivor policy: replaced lowest val_sharpe selected group "
                f"{low_survivor.get('group_id')} ({low_survivor.get('val_sharpe', 0.0):.3f}) "
                f"with non-selected group {replacement.get('group_id')} ({replacement.get('val_sharpe', 0.0):.3f})"
            )
        else:
            print(
                "  Non-BEAR survivor policy: dropped lowest val_sharpe selected group "
                f"{low_survivor.get('group_id')} ({low_survivor.get('val_sharpe', 0.0):.3f})"
            )
        print(f"  Survivors after non-BEAR policy: {len(survivors)}")
    ...
    # ------------------------------------------------------------
    portfolio_regime = np.full(
        5000, "BULL", dtype=object
    )  # Large enough buffer, will slice later
    regime_confidence = {
        i: {"BULL": 1.0, "BEAR": 0.0, "CRISIS": 0.0} for i in range(5000)
    }
    ...
    all_ticker_names = (
        [all_ticker_names[i] for i in indices_keep] if all_ticker_names else []
    )
    ...
    if n_dropped:
        ...
        all_group_ids = (
            [all_group_ids[i] for i in indices_keep] if all_group_ids else []
        )
    ...
    if sharpes_positive:
        ...
        group_weights = {
            r["group_id"]: r["val_sharpe"] / sharpes_sum
            for r in survivors
            if r["val_sharpe"] > 0
        }
    else:
    ...
    ticker_weights = np.array(
        [ticker_weight_map.get(name, 0.0) for name in all_ticker_names]
    )
    ...
    print(
        f"[Signal Weight] Range: {weight_matrix.min():.3f} - {weight_matrix.max():.3f}"
    )
    ...
    for enriched_df in all_enriched:
        try:
            ...
            if hmm_fit_cutoff is not None:
                mask = pd.to_datetime(
                    enriched_df.index, errors="coerce"
                ) <= pd.Timestamp(hmm_fit_cutoff)
    ...
    if train_enriched_dfs:
        ...
        print(
            "[RegimeDetector] No valid enriched DataFrames for HMM fitting; defaulting to BULL."
        )
        detector = None

    # Per-timestep regime confidence for GLM (default: full BULL confidence)
    regime_confidence = {
        i: {"BULL": 1.0, "BEAR": 0.0, "CRISIS": 0.0} for i in range(min_len)
    }
        ...
        try:
            ...
            for enriched_df, row_idx in zip(all_enriched, all_row_idx):
                ...
                if glm_guard is not None:
                    ticker_regimes, ticker_probs = (
                        detector.predict_regime_series_with_confidence(
    ...
    for t in range(min_len):
        ...
        for i in range(prob_matrix.shape[1]):
            ...
            if regime == "BULL":
                ...
                if (
                    prob_boosted >= long_thr
                    and long_g_matrix[t, i]
                    and not high_vol_regime[t, i]
                ):
                    signal_matrix[t, i] = 1
                elif prob_boosted <= short_thr and short_g_matrix[t, i]:
                    signal_matrix[t, i] = -1

    # Cap concurrent positions per timestep to top 6 by conviction.
    # EXP61: pair stricter conviction cap with slight BULL/BEAR entry hysteresis
    # for all-regime robustness.
    MAX_CONCURRENT = 6
    for t in range(min_len):
    ...
    if glm_guard is not None and all_lstm_probs:
        ...
        tech_indicators = {}
        for col_idx, (enriched_df, row_idx) in enumerate(
            zip(all_enriched, all_row_idx)
        ):
                ...
                if raw_idx < len(enriched_df):
                    ...
                    tech_indicators[(col_idx, t_offset)] = {
                        ...
                        "vol_percentile": round(
                            float(row.get("vol_percentile_60", 0)), 0
                        ),
                    }
        ...
        for col_idx, headlines_list in enumerate(all_headlines):
            aligned = (
                headlines_list[-min_len:]
                if len(headlines_list) >= min_len
                else headlines_list
            )
        ...
        print(
            f"[GLM] Built enrichment context: {len(tech_indicators)} technical snapshots, "
    ...
    if glm_guard is not None:
            ...
            if glm_context is not None:
                ...
                for i, ticker in enumerate(all_ticker_names):
                    ...
                    td["sentiment"] = {
                        ...
                        "headline_count": int(
                            np.expm1(max(0, glm_context["sent_vol"][t, i]))
                        ),
                    }
        ...
        print(
            f"[GLM] Hook 1 vetoed {glm_vetoed} signals "
    ...
    print("=" * 60)
    print(f"Starting Capital:      GBP{metrics['initial_capital']:,.2f}")
    print(f"Final Capital:         GBP{metrics['final_equity']:,.2f}")
    ...
    if all_group_ids:
        ...
        print("=" * 60)
        print(
            f"  {'Group':<8} {'Tickers':<28} {'Return%':>9} {'Sharpe':>8} {'MaxDD%':>8} {'Trades':>7} {'Win%':>6}"
        )
        print(f"  {'-' * 76}")
        per_group_rows = []
        ...
        if per_group_rows:
            ...
            top_share = (
                r_max / sum(r for r in returns if r > 0) * 100.0
                if sum(r for r in returns if r > 0) > 0
                else 0.0
            )
            print(
                f"  Top group: group_{top_gid}  ({r_max:+.2f}%)   "
    ...
    try:
        ...
        bh_sharpe = (
            (float(np.mean(bh_returns)) / bh_vol * np.sqrt(1638)) if bh_vol > 0 else 0.0
        )
    ...
    if "glm_vetoes" in metrics:
        ...
        print(
            f"  Hook 2 -- Trades resized:               {metrics['glm_size_adjustments']}"
        )
    ...
    if oos_holdout_pct > 0.0:
        ...
        if oos_len < 50:
            ...
            print(f"\n{'=' * 60}")
            print(
                f"OUT-OF-SAMPLE BACKTEST (last {oos_holdout_pct:.0f}% = {oos_len} timesteps)"
            )
            print(f"{'=' * 60}")
            ...
            oos_metrics = engine_oos.run_portfolio_backtest(
                price_oos, signal_oos, prob_oos
            )
            ...
            # Compute degradation
            degradation = {
                "sharpe_degradation": metrics["sharpe_ratio"]
                - oos_metrics["sharpe_ratio"],
                "return_degradation": metrics["total_return_pct"]
                - oos_metrics["total_return_pct"],
                "winrate_degradation": metrics["win_rate_pct"]
                - oos_metrics["win_rate_pct"],
                "maxdd_degradation": metrics["max_drawdown_pct"]
            ...
            print(f"  {'-' * 58}")
            print(
                f"  {'Sharpe':<20}  {metrics['sharpe_ratio']:>10.3f}  {oos_metrics['sharpe_ratio']:>10.3f}  {degradation['sharpe_degradation']:>+14.3f}"
            )
            print(
                f"  {'Return (%)':<20}  {metrics['total_return_pct']:>10.2f}  {oos_metrics['total_return_pct']:>10.2f}  {degradation['return_degradation']:>+14.2f}"
            )
            print(
                f"  {'Win Rate (%)':<20}  {metrics['win_rate_pct']:>10.1f}  {oos_metrics['win_rate_pct']:>10.1f}  {degradation['winrate_degradation']:>+14.1f}"
            )
            print(
                f"  {'Max Drawdown (%)':<20}  {metrics['max_drawdown_pct']:>10.2f}  {oos_metrics['max_drawdown_pct']:>10.2f}  {degradation['maxdd_degradation']:>+14.2f}"
            )
            print(
                f"  {'Total Trades':<20}  {metrics['total_trades']:>10d}  {oos_metrics['total_trades']:>10d}  {metrics['total_trades'] - oos_metrics['total_trades']:>+14d}"
            )
            print(f"  {'-' * 58}")
            if degradation["sharpe_degradation"] > 0.5:
                print(
                    f"  [!] WARNING: Sharpe degradation > 0.5 -- possible overfitting"
                )
...
def _monte_carlo_robustness(
    ...
    base_metrics = engine_base.run_portfolio_backtest(
        price_matrix, signal_matrix, prob_matrix
    )
    ...
    print(
        f"  Base Sharpe: {base_sharpe:.3f} | Base Return: {base_metrics['total_return_pct']:+.2f}%"
    )
    ...
    for sim in range(n_sims):
        ...
        bootstrapped = _circular_block_bootstrap(
            timestep_returns, block_size=block_size
        )
    ...
    if pbo_interpretation == "conservative" and robust_95:
        print(
            f"  [OK] MC Sharpe mean ({mc_mean:.3f}) > Original ({base_sharpe:.3f}) -- result is conservative"
        )
        print(f"  [OK] 5th percentile of MC Sharpe > 0 -- strategy is robust")
    elif pbo_interpretation == "conservative" and not robust_95:
        print(
            f"  [!] MC Sharpe mean ({mc_mean:.3f}) > Original ({base_sharpe:.3f}) -- result is conservative"
        )
        print(
            f"  [X] 5th percentile of MC Sharpe <= 0 -- some resampled scenarios lose money"
        )
    elif pbo_interpretation == "optimistic" and robust_95:
        print(
            f"  [!] MC Sharpe mean ({mc_mean:.3f}) < Original ({base_sharpe:.3f}) -- result may be optimistic"
        )
        print(f"  [OK] 5th percentile of MC Sharpe > 0 -- strategy is still robust")
    else:
        print(
            f"  [X] MC Sharpe mean ({mc_mean:.3f}) < Original ({base_sharpe:.3f}) -- possible overfitting"
        )
...
def run_compare(
            ...
            try:
                ...
                if first_ticker:
                    ...
                    _, splits, _ = _make_preprocessor(
                        LOOKBACK, HORIZON, enriched, train_anchor, use_full_split
                    )
    ...
    with open(comp_csv, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["group_id", "lstm_val_sharpe", "mtl_val_sharpe", "winner"]
        )
        writer.writeheader()
    ...
    print(
        "Equity curves: results/equity_curve_lstm.png  |  results/equity_curve_mtl.png"
    )
...
def train_or_load_saml(
    ...
    if meta_weights_path.exists():
        ...
        if first_ticker:
            ...
            n_features = splits["X_train"].shape[2]
            saml_learner = SAMLMetaLearner.load(
                str(saml_dir), input_shape=(24, n_features)
            )
            return saml_learner
    ...
    for group in groups:
        for ticker in group:
            try:
                ...
                ticker_enriched = FeatureEngineer().compute_indicators(
                    ticker_loader.load_data()
                )
                ticker_preprocessor = DataPreprocessor(
                ...
                sharpe = ticker_enriched["Close_to_SMA20"].values
                ...
                for regime_idx, regime_name in enumerate(["BULL", "BEAR", "CRISIS"]):
                    ...
                    for i, seq_start_idx in enumerate(ticker_splits["idx_train"]):
                        ...
                        if (
                            data_idx < len(regime_labels)
                            and regime_labels[data_idx] == regime_idx
                        ):
                    ...
                    if len(regime_seq_indices) > 0:
                        ...
                        if len(X_regime) > max_samples:
                            indices = np.random.choice(
                                len(X_regime), max_samples, replace=False
                            )
...
def train_or_load_quantile_barriers(
    ...
    if barrier_path.exists():
        ...
        print(
            f"[QuantileBarrier] Loaded tau_bull={learner._init_tau_bull:.3f}, "
    ...
    for group_id, stock_list in enumerate(
        groups[:5]
    ):  # Use first 5 groups for regime training
        for ticker in stock_list[:2]:  # Use first 2 tickers per group
            try:
                ...
                if anchor_end_date is not None:
                    mask = pd.to_datetime(
                        raw_df.index, errors="coerce"
                    ) <= pd.Timestamp(anchor_end_date)
        ...
        for ticker in stock_list:
            try:
                ...
                if regime_detector is not None:
                    ...
                    sharpe = enriched["Close_to_SMA20"].values
                ...
                for horizon in [4, 8, 12, 16]:  # Multiple horizons for robustness
                    if len(closes) > horizon + 10:
                        ...
                        valid_mask = ~(
                            ...
                            | np.isnan(aligned_atr)
                            | np.isnan(aligned_closes)
                        )
    ...
    if len(all_returns) < 1000:
        print(
            f"[QuantileBarrier] WARNING: Insufficient data ({len(all_returns)} samples). Using defaults."
        )
        learner = QuantileBarrierLearner(
            tau_bull=0.35, tau_bear=0.55, tau_crisis=0.70, trainable=False
        )
    ...
    optimal_taus = []
    for mask, target_q, name in [
        (bull_mask, target_quantiles[0], "BULL"),
        ...
        if np.sum(mask) > 100:
            ...
            print(
                f"  {name}: n={np.sum(mask)}, target_q={target_q:.2f}, "
    ...
    print(
        f"[QuantileBarrier] Creating learner with tau_bull={tau_bull:.3f}, "
...
def detect_primary_regime(
    train_anchor: Optional[str], test_anchor: Optional[str]
) -> str:
    """
...
def run_specialist_training(
    ...
    print(
        f"[Hybrid] Primary regime: {primary_regime} (determines which specialists to train)"
    )
    ...
    for survivor in survivors:
        ...
        for ticker in stock_list:
            try:
                ...
                primary_regime_idx = {"BULL": 0, "BEAR": 1, "CRISIS": 2}.get(
                    primary_regime, 0
                )
                regime_train_ticker = np.full(
                    len(X_train_ticker), primary_regime_idx, dtype=int
                )
                ...
                regime_val_ticker = np.full(
                    len(X_val_ticker), primary_regime_idx, dtype=int
                )
        ...
        print(
            f"[Hybrid] Training specialists: {specialists_to_train} (primary={primary_regime})"
        )
        ...
        for specialist_name in specialists_to_train:
            ...
            try:
                ...
                specialist = RegimeSpecialistLSTM(
                    ...
                    base_model_dir=str(
                        specialists_dir / specialist_name / f"group_{group_id}"
                    ),
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Hierarchical grouped training pipeline"
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
        help="Global RNG seed (vary to test different stock draws)",
    )
    parser.add_argument("--n_stocks", type=int, default=100)
    parser.add_argument("--group_size", type=int, default=5)
    parser.add_argument(
        "--threshold", type=float, default=0.0, help="Min val Sharpe to survive"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="FinBERT sentiment gate weight (0=off, 0.25=recommended)",
    )
    parser.add_argument(
        "--bear",
        action="store_true",
        help="Retrain and evaluate on 2018 Q4 bear market window",
    )
    parser.add_argument(
        "--covid",
        action="store_true",
        help="Retrain and evaluate on 2020 Q1-Q2 COVID crash window",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="Use full dataset (60/20/20 proportional split)",
    )
    parser.add_argument(
        "--live",
        action="store_true",
        help="Fetch recent data via yfinance (2yr hourly) and evaluate on present-day market (13/5/6 month split)",
    )
    parser.add_argument(
        "--custom",
        type=str,
    ...
    parser.add_argument(
        "--compare",
        action="store_true",
        help="Run both LSTM and MTL on the same groups and print a side-by-side comparison",
    )
    parser.add_argument(
        "--mtl",
        action="store_true",
    ...
    if args.custom and (args.bear or args.covid or args.full or args.live):
        parser.error(
            "--custom cannot be combined with --bear, --covid, --full, or --live."
        )
    if args.compare and args.mtl:
        parser.error(
            "--compare already runs MTL internally; do not combine with --mtl."
        )
    ...
    split_months = (
        ...
        2,
        3,
    )  # default: DataPreprocessor defaults (train/val/test months)
    ...
    if args.live:
        ...
        if (
            yf_cache_dir.exists()
            and len(list(yf_cache_dir.glob("*_1hour.txt"))) >= args.n_stocks
        ):
        ...
        _em._val_regime_config = REGIME_CONFIG["BEAR"]
        ...
        _em._val_regime_config = REGIME_CONFIG["CRISIS"]
        ...
        try:
            ...
            parser.error(
                f"Invalid date format '{args.custom}'. Use DDMMYYYY (e.g. 31012020)."
            )
        ...
        print(
            f"    Split: {split_months[0]}mo train / {split_months[1]}mo val / {split_months[2]}mo test"
        )
        ...
        _em._val_regime_config = REGIME_CONFIG["BULL"]  # EXP15: align val thresholds
        use_full_split = False
